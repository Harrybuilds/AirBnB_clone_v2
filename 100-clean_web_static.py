...
def do_clean(number=0):
    """
    This function deletes out-of-date archives of the static files.
    Args:
        number (Any): The number of archives to keep.
    """
    archives = os.listdir('versions/')
    archives.sort(reverse=True)
    startpoint = int(number)
    if not startpoinr:
        startpoint += 1
    if startpoint < len(archives):
        archives = archives[startpoint:]
    else:
        archives = []
